...
n_samples = len(ds)
samples_per_file = math.ceil(n_samples / 3)  # ensures even if not divisible by 3
current_file = 0
current_count = 0
...
def save_current_file(current_file):
    file_name = f"{save_path}-part-{current_file+1:02d}.safetensors"
    save_file(
        {
    ...
    print(f"Saved part {current_file+1} with {len(embeddings)} samples to {file_name}")
