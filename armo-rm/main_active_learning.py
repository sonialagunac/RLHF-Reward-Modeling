...
def main(cfg: DictConfig):
    ...
    # )
    score_projection, optimizer, gating_network, optimizer_gate, scheduler_gate = get_models(
        cfg, input_dim, output_dim, device
    )
    ...
    if not cfg.experiment_name_al:
        ...
        for epoch in tqdm(range(cfg.model.epochs_regression)):
            global_step += 1
            train_regression(score_projection, optimizer, updated_train_dl, device, epoch, cfg.model, step=global_step)
            if (epoch + 1) % cfg.model.eval_every == 0:
                validate_models(score_projection, gating_network, val_dl, device, cfg.model, step=global_step)   
        ...
        for epoch in tqdm(range(cfg.model.epochs_gating)):
            global_step += 1
            train_gating(gating_network, score_projection, optimizer_gate, scheduler_gate, updated_train_dl, device, epoch, cfg.model, step=global_step)
            if (epoch + 1) % cfg.model.eval_every == 0:
                validate_models(score_projection, gating_network, val_dl, device, cfg.model, step=global_step)  
    ...
    # best_val_loss = validate_gating(gating_network, score_projection, val_dl, device, cfg.model)
    ...
    for iteration in range(cfg.model.max_iters):
        ...
        validate_models(score_projection, gating_network, val_dl, device, cfg.model, step=global_step, tag="al_val", al_iteration=iteration)
        ...
        # )
        means_c, uncertainties_c, uncertainties_p = inference_active_learning(
            gating_network, score_projection, test_dl, device
        )
